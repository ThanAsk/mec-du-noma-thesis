...
hold on;
fig = plot(Kmin:0.1:Kmax,opt_delays,"LineStyle","--","Marker",marker{i},"Color",color{i},'DisplayName',gains{idx}); %,'MarkerIndices',1:2:length(opt_delays));
...
saveas(fig,fullfile(path,['geomcompSNR' append(num2str(p_db),'K',num2str(Kmin),num2str(Kmax)) '.eps']));
hold off;
figure;
