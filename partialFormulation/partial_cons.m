...
%simulatio parameters vector [B,N,Xi,Xj,fmax]
B = params(1);
N = params(2);
Xi = params(3);
Xj = params(4);
ki = params(5);  %k = ki/B*N_o = 10^-26
kj = params(6);
fmax = params(7);
...
c(3) = log(sum(exp(z + pi)) + exp(Li+2*fi+log(ki)+log(Xi))) - log(K*pmax);
c(4) = log(sum(exp(z + pj)) + exp(Lj+2*fj+log(kj)+log(Xj))) - log(K*pmax);
